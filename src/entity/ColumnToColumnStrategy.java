...
public class ColumnToColumnStrategy implements Strategy {
    ...
    public ColumnToColumnStrategy (Point point, Card card, Map<Integer, ArrayList<Card>> columns, boolean newCardShown, boolean movedToDifferentColumn) {
        this.point = point;
        this.card = card;
    ...
    public MoveCardOutputData execute() {
        for (int i = 0; i < 7; i++) {
            ...
            if (size == 0 && 110 * i + 70 <= point.getX() && point.getX() + 100 <= 110 * i + 250 && 0 <= point.getY() && point.getY() + 140 <= 180 &&
                    card.getValue() == 13) {
                canBeMoved = true;
                movedColumn = i + 1;
                if (newCardShown) {
